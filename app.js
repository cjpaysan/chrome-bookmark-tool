...
const BUILD = 'v1.0.5 @ 2026-08-16T14:09';
const VERSION = '1.0.5'; // 语义化版本号（与 package.json 一致）
...
async function loadMoveFolders(list, bms, okBtn) {
  try {
    // 构造 query：与文件夹树（#folderTree）一致，传当前来源的 browser+profile 或 importId
    let q = '';
    if (currentScanSource) {
      if (currentScanSource.importId) q = `importId=${encodeURIComponent(currentScanSource.importId)}`;
      else if (currentScanSource.browser) q = `browser=${encodeURIComponent(currentScanSource.browser)}&profile=${encodeURIComponent(currentScanSource.profile)}`;
    }
    if (!q) {
      list.innerHTML = '<p class="muted" style="padding:12px;color:#cf222e">未识别当前来源。请重新选择书签来源并点「开始扫描」再试。</p>';
      return;
    }
    const r = await fetch(`/api/folders?${q}`, { cache: 'no-store' });
    const d = await r.json();
    if (!d.ok && !d.tree) throw new Error(d.error || '加载失败');
    const tree = d.tree || [];
    if (!tree.length) {
    ...
    const walk = (nodes, prefix) => {
      nodes.forEach((n) => {
        ...
        item.dataset.path = n.name;
        item.style.cssText += indent;
        item.innerHTML = (n.children && n.children.length ? '📁 ' : '📂 ') + `<span>${esc(n.name)}</span>`;
        item.onclick = () => {
...
async function doMoveBookmarks(bms, targetFolderPath) {
  ...
  try {
    ...
    const r = await fetch('/api/move-synced', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ items, targetFolderPath }),
    });
    const d = await r.json();
    if (d.ok) {
