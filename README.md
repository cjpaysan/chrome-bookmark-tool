...
- ⏸️ **扫描可暂停 / 继续**：大批量扫描时随时暂停，处理完再继续，不用从头来过。
- 💾 **扫描断点续扫**：扫描中途关掉应用或崩溃，下次打开会自动提示从上次进度接着扫。
- 🗂 **删前自动备份**：删除同步书签前自动导出带时间戳的 Chrome 可导入 HTML，删错可在 Chrome 一键导入恢复。
- ↩️ **应用内撤销**：删除 / 移动书签后，在成功弹窗点「撤销」即可原地还原回原文件夹。
...
> 版本号格式 `主.次.修.构建`，构建位每次打包自动 +1；以下为近期主要更新。

### 🆕 v1.2.0.9 · 应用内撤销
- 删除 / 移动同步书签后，成功弹窗提供「↩ 撤销」按钮，一键还原回原文件夹。
- 撤销缓冲持久化到本地（`output/.undo.json`），**重启应用后仍可撤销最近一次操作**。

### 🆕 v1.2.0.8 · 删前自动备份
- 删除同步书签前自动导出带时间戳的 Netscape HTML（Chrome 可直接「导入书签」），成功弹窗提供下载链接，删错可恢复。

### 🆕 v1.2.0.7 · 登录识别优化
- 失效检测区分「需登录才能访问」与「真的失效」：需登录的页面不再误判为失效，清理更精准。

### 🆕 v1.2.0.6 · 断点续扫
- 扫描会话落盘，应用关闭 / 崩溃后可续扫，不必从头再来。

### 🆕 v1.2.0.5 · 扫描暂停 / 继续
- 扫描过程中可随时暂停，处理完再继续（取消能力原有）。

### 🆕 v1.2.0.4 · 安装指引傻瓜化
- 新增一键 `install.command`（自动去拦截 + 装进「应用程序」+ 启动）、图文安装指引。
- 清理调试接口与冗余代码，仓库更干净。
